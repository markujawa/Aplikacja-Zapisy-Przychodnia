...
/**
 * Klasa reprezentująca okienko umożliwiające ustawienie hasla przez lekarza
 */
public class SetPasswordDoctor implements ActionListener {
    ...
    /**
     * Konstruktor klasy SetPasswordDoctor
     */
    public SetPasswordDoctor() {
     ...
     */
    public void insertDoctorPassword(Password pass) {
        String sql = "UPDATE doctors_login_data SET haslo = ? WHERE (haslo= 'brak')";

        try (Connection conn = ConnectionDB.connectDB();
                PreparedStatement pstmt = conn.prepareStatement(sql)) {
            pstmt.setString(1, pass.getPassword());
            pstmt.executeUpdate();
            System.out.println("Dodano hasło do bazy lekarzy!");
        ...
        if (ae.getSource() == setButton) {
            ...
            try {
                ...
                if (passwordField.getText().equals(passwordField2.getText())) {
                    insertDoctorPassword(pass);
                    /**
                     * Wywołanie konstruktora LegitDoctor
                     */
                    LegitDoctor ld = new LegitDoctor();
                    frame.dispose();
                } else {
